...
public $column_align ;
	...
	// Add Invoice Number
	function AddInvoiceNumber($invoiceNumber) 
	{
	    // Positionnement à 10 cm 
	    $this->SetXY(150,5);
	    $this->SetFont('Arial','',10);
		$this->MulticellUtf8(35, 5, "Facture Numéro\n".$invoiceNumber, 1, 'C', false);
	}
	// Add Invoice Number
	function AddAddress($name, $address, $city, $country) 
	{
		print("function AddAddress($name, $address, $city, $country) </br>");
		$text=db2web($name."\n".$address."\n".$city."\n".$country);
	    // Positionnement à 10 cm 
	    $this->SetXY(100,35);
	    $this->SetFont('Arial','',10);
		//$this->MulticellUtf8(100, 6, $name."\n".$address."\n".$city."\n".$country, 1, 'L', false);
		$this->MulticellUtf8(100, 6, $text, 1, 'L', false);
	}
