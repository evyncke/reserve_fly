...
<?php
    foreach($folio as $line) {
	    ...
	    if ($line->cost_plane > 0) {
            $ximportLine = new XimportLine($nextMove, 'VEN', $line->date, '', $nextInvoice, 700100, $libelle, $line->cost_plane, 'C', $nextInvoice,
        ...
        if ($line->cost_taxes > 0) {
            ...
            $ximportLine = new XimportLine($nextMove, 'VEN', $line->date, '', $nextInvoice, 744103, $libelle, $line->cost_taxes, 'C', $nextInvoice,
                'TI', 0.0, 0.0, '0', $line->date, 0) ;
            fprintf($f, "$ximportLine\n") ;        
			
			$taxPerPaxText=number_format($tax_per_pax,2,",",".")." €";
        ...
        if ($line->cost_fi > 0) {
            ...
            $ximportLine = new XimportLine($nextMove, 'VEN', $line->date, '', $nextInvoice, $fi_code, $libelle, $line->cost_fi, 'C', $nextInvoice,
    ...
    $ximportLine = new XimportLine($nextMove, 'VEN', $line->date, '', $nextInvoice, $cielCode400, $libelle , $total_folio, 'D', $nextInvoice,
        '', 0.0, 0.0, ' ', '', 0) ;
    fprintf($f, "$ximportLine\n") ;
	...
	$pdf->AddProlog();
	$pdf->Output('F', "data/PDFInvoices/Invoice_".$lastName.".pdf");
}
