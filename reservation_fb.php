...
$result = mysqli_query($mysqli_link, "select upper(id) as id, classe, compteur, compteur_date, entretien, photo, sous_controle, delai_reservation, commentaire, actif, compteur_vol
	from $table_planes
...
//		order by l_end desc") or die("Cannot get last reservation: " . mysqli_error($mysqli_link)) ;
	...
	print("{ \"id\": \"$row[id]\", \"compteur\": $row[compteur], \"compteur_date\": new Date(\"" . str_replace('-', '/', $row['compteur_date']) . "\"), " .
		...
		" \"dernier_vol\": \"$l_end\", \"actif\": $row[actif], " .  
...
if ($userId == 0) {
	...
	print('<input type="button" value="Mobile" onclick="javascript:document.location.href=\'booking.php?me=' . $userId .
		'&auth=' . md5($userId . $shared_secret) . '\';"> ') ;
...
// Verify non-logged flights in the last week
	$result = mysqli_query($mysqli_link, "select * from $table_bookings b 
		where (b.r_pilot = $userId or b.r_who = $userId or b.r_instructor = $userId) and
...
if (mysqli_num_rows($result_news) or $userIsAdmin) {
	print('<td class="hidden-phone"><div id="newsDiv"><ul>') ;
...
?>
		...
		<td id="webcamCell" class="hidden-phone">
			<a href="" id="webcamURI" border="0"><img id="webcamImg" width="128" hight="96" alt="Webcam" onStalled="imgStalled();"></a>
		</td>
	</tr>
</table>
<br/>
...
<br/>
Lieu de d&eacute;part: <input type=text id="departingAirport" size="4" maxlength="4" onKeyUp="airportChanged(this);" value="EBSP">
d'arriv&eacute;e: <input type=text id="destinationAirport" size="4" maxlength="4" onKeyUp="airportChanged(this);"> <i>(codes OACI)</i>
<br/>
Via: <input type=text id="via1Airport" size="4" maxlength="4" onKeyUp="airportChanged(this);">
et: <input type=text id="via2Airport" size="4" maxlength="4" onKeyUp="airportChanged(this);"> <i>(codes OACI)</i>
<br/>
<hr>
...
<!-- end of div for the booking window-->
