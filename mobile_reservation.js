		...
		if(this.readyState  == 4) {
			if(this.status  == 200) {
				...
				if (metar_response.error != '') {
					...
					if ((userId == 62 || userId == 102) && metar_response.temperature &&  metar_response.dew_point && (( metar_response.temperature -  metar_response.dew_point) <= 10))
						elem.innerHTML += '<br/><i>Relative humidity: ' +
							Math.round(100.0 - 5.0 * ( metar_response.temperature - metar_response.dew_point)) + '%</i>' ;
					if ((userId == 62 || userId == 102) && metar_response.clouds_base) {
						elem.innerHTML += '<br/><i>Theorical clouds base at ' +  metar_response.station + ': ' +  metar_response.clouds_base + ' ft</i>' ;
					}
					if ((userId == 62 || userId == 102) && metar_response.density_altitude) {
	...
	if (planningByPlane) { // Only one plane is displayed but for several days
		...
		for (var day = 0; day < 7;  day++) {
			...
			for (i = 1; i < columnCount; i ++) {
				newRow.insertCell(i).className = 'slot available' ; // Would be nice to care for available_for_fi as well...
			}
		}
		// hide instructorPlanningTable
		instructorPlanningTable.style.visibility = 'hidden' ;
		...
		for (var plane = 0; plane < allPlanes.length; plane++) {
			...
			for (i = 1; i < columnCount; i ++) {
				switch (allPlanes[plane].actif) {
					case 0: newRow.insertCell(i).className = 'slot other maintenance' ; break ; // Anyway, this row is hidden
					case 1: newRow.insertCell(i).className = (allPlanes[plane].ressource != 0) ? 'slot ressource_available' : 'slot available' ; break ;
					case 2: newRow.insertCell(i).className = 'slot available_for_fi' ; break ;
				}
			}
		}
...
function cancelBooking(bookingIsForFlying) {
	...
	XHR.onreadystatechange = function() {
		...
		if (this.readyState  == 1) { // First of 4 steps, start waiting, TODO should probably be moved outside
			jQuery("#bookingMessageModal").show() ;
			displayWaiting() ;
...
function editBookingDetails(event) {
	...
	var booking = allBookings[bookingFromID(id)][loggingFromID(id)] ;
	document.getElementById('bookingTitle').innerHTML = "Annuler/modifier une réservation" ; 
...
// Allow resizable bookings per https://stackoverflow.com/questions/26233180/resize-a-div-on-border-drag-and-drop-without-adding-extra-markup
	...
	for (var i = 1, hour = planningStartHour, minute = 0 ; i < columnCount ; i ++, minute += 15) {
		...
		if (startDate <= workDate && workDate < endDate) {
			if (nameDisplayed) {
				...
				else if (typeof booking.blocked_date !== 'undefined') // TODO only for future bookings ?
		...
		if (startDate <= workDate && workDate < endDate) {
			...
			if (item.booking) { // A little tricky because the allBookings[] is built asynchronously... so we need to handle this case
				...
				if (nameDisplayed) {
					widthInColumn++ ;
					if (item.callType < 0)
						thisCell.className = 'slot unavailable other' ;
					else
						thisCell.className = (item.callType & 0x01) ? 'slot onsite other' : 'slot oncall other' ; 
				} else { // Name not yet displayed, need to display some more information
					nameDisplayed = true ;
					firstColumn = i ;
					widthInColumn = 1 ;
					...
					if (item.callType < 0)
						thisCell.className = 'slot unavailable first' ;
					else
						thisCell.className = (item.callType & 0x01) ? 'slot onsite first' : 'slot oncall first' ; 
				}
				if (!isInThePast(displayYear, displayMonth, displayDay, hour, minute) &&
...
function refreshPlanePlanningRow(rowIndex, plane, displayDay, displayMonth, displayYear) {
	...
	for (var i = 1, hour = planningStartHour, minute = 0; hour < planningStopHour ; i++, minute += 15) {
		...
		if (hour > aeroSunsetHours ||
			...
			row.cells[i].className = 'slot night first' ;
			row.cells[i].removeEventListener('click', newBookingDetails) ;
		} else {
			if (isNow(displayYear, displayMonth, displayDay, hour, minute))
				row.cells[i].className = 'slot now first' ;
			else {
				for (var j = 0; j < allPlanes.length; j++) {
					if (allPlanes[j].id == plane) 
						if (allPlanes[j].ressource == 0)
							switch (allPlanes[j].actif) {
								case 1: row.cells[i].className = 'slot available first' ; break ;
								case 2: row.cells[i].className = 'slot available_for_fi first' ; break ;
							}
						else
							row.cells[i].className = 'slot ressource_available first' ;
				}
			}
	...
	for (var i = 1, hour = planningStartHour, minute = 0; hour < planningStopHour ; i++, minute += 15) {
		...
		if (hour > aeroSunsetHours ||
			...
			if (isNow(displayYear, displayMonth, displayDay, hour, minute))
				...
				row.cells[i].className = 'slot available first' ;
			// Probably cannot pass 'plane' and 'i' as parameter for some javascript parameter binding
			// Need to set the TD cell id?
			// Should place the plane index?
